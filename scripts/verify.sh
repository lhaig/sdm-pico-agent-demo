...
if admin_db_up; then
    ...
    READ_GRANTS="$(psql_admin_q "SELECT has_table_privilege('shopfront_read', 'public.orders', 'SELECT') AND has_table_privilege('shopfront_read', 'public.poison_backup', 'SELECT')" | tr -d '[:space:]')"
    if [[ "$READ_GRANTS" == "t" ]]; then pass "standing identity can inspect the reversible order backup"; else fail "standing identity cannot inspect the remediation predicate"; fi
